...
import { UploadStatementSheet } from '@/components/forms/UploadStatementSheet'
import { AddTransactionSheet } from '@/components/forms/AddTransactionSheet'
import { AddInvestmentSheet } from '@/components/forms/AddInvestmentSheet'

type ActiveSheet = 'upload' | 'transaction' | 'investment' | null
...
export function FAB() {
  ...
  const actions = [
    { icon: Upload, label: 'Upload Statement', sheet: 'upload' as ActiveSheet, color: 'bg-blue-500' },
    { icon: PenLine, label: 'Add Transaction', sheet: 'transaction' as ActiveSheet, color: 'bg-green-500' },
    { icon: TrendingUp, label: 'Add Investment', sheet: 'investment' as ActiveSheet, color: 'bg-purple-500' },
  ]
  ...
  return (
    <>
      <div ref={ref} className="fixed bottom-20 right-4 z-50 flex flex-col-reverse items-end gap-2">
        <AnimatePresence>
          {open &&
            actions.map(({ icon: Icon, label, sheet, color }, i) => (
              <motion.button
                key={label}
