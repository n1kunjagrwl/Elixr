...
class TestGetCategories:
    async def test_get_categories_returns_200_with_list(self):
        ...
        settings = make_test_settings()
        categories = [
            _make_category_response(
                name="Food & Dining", is_default=True, user_id=None
            ),
            _make_category_response(
                name="My Custom", is_default=False, user_id=USER_ID
            ),
        ]
    ...
    async def test_post_category_transfer_kind_returns_403(self):
        ...
        svc = _make_mock_service(
            create_category=AsyncMock(
                side_effect=CategoryKindForbiddenError(
                    "Transfer categories not allowed"
                )
            )
        )
        app = _build_cat_app(svc, settings)
    ...
    async def test_patch_category_default_returns_403(self):
        ...
        svc = _make_mock_service(
            update_category=AsyncMock(
                side_effect=CannotEditDefaultCategoryError("Cannot edit defaults")
            )
        )
        app = _build_cat_app(svc, settings)
    ...
    async def test_get_rules_returns_200(self):
        ...
        async with AsyncClient(
            ...
            resp = await client.get(
                "/categorization-rules", headers=_make_auth_header(settings)
            )
...
class TestPostRule:
    async def test_post_rule_returns_201(self):
        ...
        settings = make_test_settings()
        cat_id = uuid.uuid4()
        response_obj = _make_rule_response(
            pattern="zomato", match_type="contains", category_id=cat_id
        )
