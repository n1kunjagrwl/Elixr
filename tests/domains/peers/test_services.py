...
class TestAddContact:
    async def test_add_contact_creates_contact(self, mock_db):
        ...
        with patch.object(svc._repo, "create_contact", new=AsyncMock(return_value=contact)):
...
class TestEditContact:
    async def test_edit_contact_updates_fields(self, mock_db):
        ...
        with patch.object(svc._repo, "get_contact", new=AsyncMock(return_value=contact)), \
...
class TestDeleteContact:
    async def test_delete_contact_with_open_balances_raises_409(self, mock_db):
        ...
        with patch.object(svc._repo, "get_contact", new=AsyncMock(return_value=contact)), \
             patch.object(svc._repo, "has_open_balances", new=AsyncMock(return_value=True)):
    ...
    async def test_delete_contact_no_balances_succeeds(self, mock_db):
        ...
        with patch.object(svc._repo, "get_contact", new=AsyncMock(return_value=contact)), \
             patch.object(svc._repo, "has_open_balances", new=AsyncMock(return_value=False)), \
...
class TestLogBalance:
    async def test_log_balance_creates_balance(self, mock_db):
        ...
        with patch.object(svc._repo, "get_contact", new=AsyncMock(return_value=contact)), \
             patch.object(svc._repo, "create_balance", new=AsyncMock(return_value=balance)):
...
class TestEditBalance:
    async def test_edit_balance_updates_description_and_notes(self, mock_db):
        ...
        data = PeerBalanceUpdate(description="Updated dinner split", notes="Split evenly")

        with patch.object(svc._repo, "get_balance", new=AsyncMock(return_value=balance)), \
...
class TestListBalances:
    async def test_list_balances_filters_by_status(self, mock_db):
        ...
        with patch.object(svc._repo, "list_balances", new=AsyncMock(return_value=[open_balance])):
...
class TestRecordSettlement:
    async def test_record_settlement_updates_settled_amount_and_status_to_partial(self, mock_db):
        ...
        with patch.object(svc._repo, "get_balance", new=AsyncMock(return_value=balance)), \
             patch.object(svc._repo, "create_settlement", new=AsyncMock(return_value=settlement)), \
             patch.object(svc._repo, "update_balance_settled_amount", new=AsyncMock(return_value=None)), \
             patch.object(svc._repo, "refresh_balance", new=AsyncMock(return_value=updated_balance)):
...
class TestListSettlements:
    async def test_list_settlements_returns_settlements_for_balance(self, mock_db):
        ...
        with patch.object(svc._repo, "get_balance", new=AsyncMock(return_value=balance)), \
             patch.object(svc._repo, "list_settlements", new=AsyncMock(return_value=[settlement1, settlement2])):
